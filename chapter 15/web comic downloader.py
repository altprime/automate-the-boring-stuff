...
downlaod all web comics since last run
list of web comics available on http://nostarch.com/automatestuff/
...
def check_xkcd(last_url):
    ...
    url = 'https://xkcd.com'
    # get latest comics url with comic number for comparison/update
    ...
    if url == last_url:
        print('No new comics have been published since last check')
    else:
        while url != last_url:
            res = requests.get(url)
            res.raise_for_status()
